"""
whatsapp_watcher.py — WhatsApp Watcher for the Platinum-tier AI Employee.

Platinum change: writes to Needs_Action/email/ (WhatsApp is a Local-agent
channel — Local owns the WhatsApp session per the Platinum spec).

Polls WhatsApp Web for unread messages containing business-relevant keywords.
Extends BaseWatcher from base_watcher.py (Bronze/Silver/Platinum pattern).
...
Architecture:
    Playwright → WhatsApp Web → Needs_Action/email/ → Cloud Agent → Done/
...
HITL: WhatsApp sends (replies) are NEVER made by this watcher.
    Drafts are written to Needs_Action/email/ for Cloud Agent to process.
    New contacts and messages with financial content are escalated to
    Pending_Approval/ by SKILL_Gmail_Triage.
...
Usage:
    ...
    python whatsapp_watcher.py --vault /path/to/platinum --interval 60
...
class WhatsAppWatcher(BaseWatcher):
    ...
    FILTER_KEYWORDS, and writes structured .md files to Needs_Action/email/.
    ...
    Args:
        vault_path:           Root directory of the Platinum vault.
        session_path:         Directory where Playwright stores browser session
                              data (cookies, localStorage). Created automatically.
                              Default: <vault_path>/whatsapp_session
        check_interval:       Seconds between WhatsApp polls. Default 60.
        headless:             Run Chromium without a visible window. Set False
                              for first run to scan the QR code. Default True.
        keywords:             Keyword filter list. Default FILTER_KEYWORDS.
        needs_action_subdir:  Sub-folder inside Needs_Action/ to write to.
                              Default "email" (Platinum: WhatsApp messages are
                              treated as communications alongside email).
    """
    ...
    def create_action_file(self, message: dict) -> Optional[Path]:
        """
        Write a structured .md file to Needs_Action/email/ for one WhatsApp message.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "WhatsAppWatcher — Platinum AI Employee WhatsApp perception layer.\n\n"
    ...
    parser.add_argument(
        "--vault",
        default=".",
        help="Path to Platinum vault root (default: current directory)",
    )
    parser.add_argument(
        "--session",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Seconds between WhatsApp polls (default: 60)",
    )
    parser.add_argument(
        "--no-headless",
        action="store_true",
